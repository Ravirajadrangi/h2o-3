...
tokenize <- function(sentences, stop.words = STOP_WORDS) {
    ...
    tokenized.words[tokenized.words == NA || (! (as.factor(tokenized.words) %in% STOP_WORDS)),]
}

aggregate2vecs <- function(word.vectors, words) {
    word.vectors$C0 <- word.vectors$C1 != NA
    word.vectors[word.vectors$C0 != 1,] <- 0.0

    word.sums <- h2o.cumsum(word.vectors)

    r <- word.sums[words$C1 == NA,]
    if (nrow(r) > 1) {
...
h2o.findSynonyms(w2v.model, "teacher", count = 5)
...
valid.job.titles <- job.title.vecs$C0 != NaN
